...
except ImportError: from .nb_TSBasicData import TimeSeriesItem
        ...
        for cb in learn.callback_fns:
            ...
            if hasattr(cb, 'keywords') and hasattr(get_fn(cb), 'on_batch_begin'):
                ...
                try:
                    ...
                    [Image(Tensor2ImgTensor(cb_fn(learn).on_batch_begin(xb, yb, True)['last_input'][rand_int]))
                     .show(ax=ax) for i, ax in enumerate(fig)]
        ...
        for cb in learn.callback_fns:
            ...
            if hasattr(cb, 'keywords') and hasattr(get_fn(cb), 'on_batch_begin'):
                ...
                try:
                    fig = plt.subplots(rows, cols, figsize=figsize, sharex=True, sharey=True)[1].flatten()
                    #plt.suptitle(get_fn(cb).__name__, size=14)
                    [(TimeSeriesItem(cb_fn(learn).on_batch_begin(xb, yb, True)['last_input'][rand_int])
                     .show(ax=ax))
