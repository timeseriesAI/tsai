#AUTOGENERATED! DO NOT EDIT! file to edit: ./ColorfulDim.ipynb (unless otherwise specified)
...
class ActivationsHistogram(HookCallback):
    ...
    def __init__(self, learn:Learner, do_remove:bool=True,
        ...
        self.hMax = hMax
        self.nBins = nBins
        self.zero_bin = math.floor(- self.nBins * self.hMin / (self.hMax - self.hMin))
    ...
    def on_train_end(self, **kwargs):
        ...
        self.activations_histogram.plotActsHist()
    ...
    @staticmethod
    def getMin(act,useClasses,zero_bin):
        dd = act.squeeze(2) if not useClasses else act.sum(dim=2) # Reassemble...
        dd = dd.t().float() # rotate
        return dd[zero_bin - 1:zero_bin + 2].sum(0)/dd.sum(0)
    ...
    def plotActsHist(self, cols=10, toDisplay=None, hScale = .05,
                     showEpochs=False, showLayerInfo=True, aspectAuto=True, showImage=True):
        ...
        grid = plt.GridSpec(rows, cols, figure=fig ,left=None, bottom=None,
                            right=None, top=None, wspace=.25, hspace=.25)
        ...
        plt.show()
        if showLayerInfo:
            for i, l in enumerate(self.allModules): print('{:2} {}'.format(i, l))



def telemetry(learn:Learner, ah:Callable=None, modulesId:Union[bool, list]=None, filt:Callable=None, hMin:int=-10, hMax:int=10, nBins:int=200) -> Learner:
    if modulesId == None and filt != None: modulesId = get_layers_idx(learn, filt)
    if ah is None: ah = partial(ActivationsHistogram, modulesId=modulesId, hMin=hMin, hMax=hMax, nBins=nBins, liveChart=False)
    learn.callback_fns.append(ah)
    return learn

Learner.telemetry = telemetry
...
def get_layers(model, cond=noop):
    ...
    if hasattr(model, 'body') or hasattr(model, 'head'):
        all_layers = []
        if hasattr(model, 'body'):
            for i in range(len(model.body)):
                all_layers.extend(_get_layers(model.body[i], cond=cond))
        if hasattr(model, 'head'):
            for i in range(len(model.head)):
                all_layers.extend(_get_layers(model.head[i], cond=cond))
        return all_layers
    else: return _get_layers(model, cond=cond)


def get_layers_idx(learn, cond=noop):
    mods = get_layers(learn)
    idx = []
    for i,m in enumerate(mods):
        if cond(m): idx.append(i)
    return idx