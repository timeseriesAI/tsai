...
class TemporalBlock(Module):
    def __init__(self, ni, nf, ks, stride, dilation, padding, dropout=0.2):
...
def temporalconvnet(c_in, layers, ks=2, dropout=0.2):
    temp_layers = []
    for i in range(len(layers)):
        dilation_size = 2 ** i
        ni = c_in if i == 0 else layers[i-1]
        nf = layers[i]
        temp_layers += [TemporalBlock(ni, nf, ks, stride=1, dilation=dilation_size,
                                      padding=(ks-1) * dilation_size, dropout=dropout)]
    return nn.Sequential(*temp_layers)

class TCN(Module):
    def __init__(self, c_in, c_out, layers=8*[25], ks=7, conv_dropout=0.2, fc_dropout=0.):
        self.tcn = temporalconvnet(c_in, layers, ks=ks, dropout=conv_dropout)
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.fc_do = nn.Dropout(fc_dropout) if fc_dropout else None
        self.fc = nn.Linear(layers[-1],c_out)

    def forward(self, x):
        x = self.tcn(x)
        x = self.gap(x).squeeze(-1)
        if self.fc_do is not None: x = self.fc_do(x)
        return self.fc(x)