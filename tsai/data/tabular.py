"""Docs: https://timeseriesAI.github.io/tsaidata.tabular.html.md"""
...
# %% ../../nbs/014_data.tabular.ipynb #558ca47b
import fastai.torch_core as _ftc_torch
from fastai.tabular.all import *

from ..imports import *
from ..utils import *

# %% ../../nbs/014_data.tabular.ipynb #49ef1b90
# Patch as_tensor in fastai.torch_core to handle numpy arrays with negative strides
# (compatibility fix for newer pandas/PyTorch versions)
_torch_as_tensor = torch.as_tensor  # original torch C function, safe against re-execution
def _safe_as_tensor(data, *args, **kwargs):
    if isinstance(data, np.ndarray) and data.ndim > 0 and (any(s < 0 for s in data.strides) or not
data.flags.writeable):
        data = data.copy()
    return _torch_as_tensor(data, *args, **kwargs)
_ftc_torch.as_tensor = _safe_as_tensor

# %% ../../nbs/014_data.tabular.ipynb #9891d46f
...
def get_tabular_ds(df, procs=[Categorify, FillMissing, Normalize], cat_names=None, cont_names=None, y_names=None, groupby=None,
    ...
    to.items = to.items.copy()  # ensure contiguous memory layout (avoids negative stride errors with PyTorch)
    setattr(to, "groupby", groupby)
    return to

# %% ../../nbs/014_data.tabular.ipynb #8bdc4782
...
# %% ../../nbs/014_data.tabular.ipynb #71709841
