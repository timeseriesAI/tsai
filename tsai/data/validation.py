...
__all__ = ['check_overlap', 'leakage_finder', 'balance_idx', 'TrainValidTestSplitter', 'get_splits', 'TimeSplitter',
           'get_predefined_splits', 'combine_split_data', 'get_splits_len', 'get_info']
...
def get_info(X, y=None, splits=None):
    if X.ndim < 3: to3darray(X)
    print(f'X         : [samples={X.shape[0]} x features={X.shape[1]} x timesteps={X.shape[-1]}]')
    if y is not None:
        print(f'y shape   : {y.shape}')
        if isinstance(y[0], (Integral, str)): print(f'n classes : {len(np.unique(y))} {L(np.unique(y).tolist())}')
    if splits is not None: print('n_splits  :', get_splits_len(splits))