...
__all__ = ['check_overlap', 'check_splits_overlap', 'leakage_finder', 'balance_idx', 'TrainValidTestSplitter',
           'plot_splits', 'get_splits', 'TimeSplitter', 'get_predefined_splits', 'combine_split_data', 'get_splits_len']
...
# Cell
def TimeSplitter(valid_size:Union[int, float]=0.2, show_plot:bool=True):
    ...
    def _inner(o):
        ...
        idx = np.arange(len(o))
        splits = L(idx[:-cut].tolist()), L(idx[-cut:].tolist())
        if show_plot: plot_splits(splits)
        return splits
    return _inner
