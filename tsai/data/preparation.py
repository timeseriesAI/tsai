...
def delta_timestamps(data, dir='forward'):
    """Calculates time gaps to the next (forward) or last (backward) mask time steps in a pandas dataframe.
...
def forward_gaps(o, nan_to_num=-1):
    """Number of sequence steps since previous real value along the last dimension of 3D arrays or tensors"""

    b,c,s=o.shape
    if isinstance(o, torch.Tensor):
        idx = torch.where(o==o, torch.arange(s), 0)
        idx = torch.cummax(idx,axis=-1).values
        gaps = 1 + (torch.arange(s).reshape(1,1,-1).repeat(b, c, 1) - idx).float()
        mask = torch.isnan(o[torch.arange(b)[:,None, None], torch.arange(c)[None, :, None], idx])
...
def backward_gaps(o, nan_to_num=-1):
