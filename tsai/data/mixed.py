# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/012c_data.mixed.ipynb (unless otherwise specified).

__all__ = ['MixedDataLoader', 'MixedDataLoaders', 'get_mixed_dls', 'combine_dls']
...
class MixedDataLoader():
    def __init__(self, *loaders, path='.', shuffle=False, device=None, bs=None, c=None, d=None, cat=None, cont=None, classes=None):
        ...
        for i, dl in enumerate(loaders):  # ensure all dls have the same bs
            if len(dl.dataset) > 0:
                ...
                if self.d is None and hasattr(dl, "d"): self.d = dl.d
                if self.cat is None and hasattr(dl, "cat_names"): self.cat = len(dl.cat_names)
                if self.cont is None and hasattr(dl, "cont_names"): self.cont = len(dl.cont_names)
                if self.classes is None and hasattr(dl, "classes"): self.classes = dl.classes
            ...
            dl.to(device=device)
        if self.c is None: self.c = c
        if self.d is None: self.d = d
        if self.cat is None: self.cat = cat
        if self.cont is None: self.cont = cont
    ...
    def _get_vals(self, x):
        ...
        for idx, o in enumerate(x):
            if o != [] and not self._arrayisin(o, new_x):
                idxs.append(idx)
                new_x.append(o)
        return idxs
    ...
    def _get_idxs(self):
        ...
        # self.x_idxs = [item for sublist in self.x_idxs for item in listify(sublist)]
        ...
        # self.y_idxs = [item for sublist in self.y_idxs for item in listify(sublist)]
...
class MixedDataLoaders(DataLoaders):
    def __init__(self, *loaders, path='.', device=None, c=None, d=None, cat=None, cont=None, classes=None):
        self.loaders,self.path = list(loaders),Path(path)
        if device is not None or hasattr(loaders[0],'to'): self.device = device
        self.c, self.d, self.cat, self.cont, self.classes = c, d, cat, cont, classes
...
# Cell
def combine_dls(*loaders, device=None, path='.', bs=None):
    _mixed_train_dls = []
    _mixed_valid_dls = []
    for dls in loaders:
        _mixed_train_dls.append(dls.train)
        _mixed_valid_dls.append(dls.valid)
        if device is None: device = dls.train.device
    mixed_train_dls = MixedDataLoader(*_mixed_train_dls, bs=bs)
    mixed_valid_dls = MixedDataLoader(*_mixed_valid_dls, bs=bs)
    mixed_dls = MixedDataLoaders(mixed_train_dls, mixed_valid_dls, device=device)
    return mixed_dls