...
__all__ = ['decompress_from_url', 'get_UCR_univariate_list', 'UTSC_datasets', 'UCR_univariate_list',
...
# Cell
def get_UCR_data(dsid, path='.', parent_dir='data/UCR', on_disk=True, mode='c', return_split=True, split_data=True, force_download=False, verbose=False):
    ...
    if force_download or not all([os.path.isfile(f'{full_tgt_dir}/{fn}.npy') for fn in ['X_train', 'X_valid', 'y_train', 'y_valid', 'X', 'y']]):
        ...
        X_train_ = []
        X_valid_ = []
        for i in progress_bar(range(X_train_df.shape[-1]), leave=False, comment=f'preparing {dsid} data'):
...
# Cell
def get_Monash_data(dsid, path='./data/Monash', on_disk=True, mode='c', split_data=True, force_download=False, verbose=False):
