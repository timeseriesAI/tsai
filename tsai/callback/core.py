...
__all__ = ['GamblersCallback', 'TransformScheduler', 'ShowGraph', 'ShowGraphCallback2', 'UBDAug',
           'WeightedPerSampleLoss', 'BatchResampler']
...
class BatchResampler(Callback):
    """ Callback that selects a random percentage of samples and/ or selects a random percentage of sequence steps
        from each training batch (with or without replacement)
    """

    def __init__(self, sample_pct=None, sample_replace=True, step_pct=None, step_replace=True, sort_steps=True):
        store_attr()
    ...
    def before_batch(self):
        ...
        if self.sample_pct is not None:
            B = self.x.shape[0]
            idxs = np.random.choice(B, round(B * self.sample_pct), self.sample_replace)
        ...
        if self.step_pct is not None:
            S = self.x.shape[-1]
            idxs = np.random.choice(S, round(S * self.step_pct), self.step_replace)
            if self.sort_steps: idxs = np.sort(idxs)
            self.learn.xb = tuple(xbi[...,idxs] for xbi in self.learn.xb)